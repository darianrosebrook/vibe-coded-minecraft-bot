import { Task, TaskProgress, TaskResult } from '../types/task';
...
export class ErrorHandler {
  ...
  public async handleError(error: Error, context: ErrorContext): Promise<void> {
    ...
    if (strategy && strategy.shouldRetry(context)) {
      const delay = Math.min(
        strategy.baseDelay * Math.pow(strategy.backoffFactor, context.retryCount),
        strategy.maxDelay
      );

      console.log(`Retrying ${context.category} error (attempt ${context.retryCount + 1}/${strategy.maxRetries}) after ${delay}ms`);
    ...
    throw new Error(`Failed to handle ${context.category} error after all retries and fallbacks: ${error.message}`);
  }
  ...
  public categorizeError(error: Error): ErrorCategory {
    ...
    if (error.message.includes('path') || error.message.includes('navigation')) {
      return 'PATHFINDING';
    }
