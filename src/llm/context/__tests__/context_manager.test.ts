import { ContextManager, GameState, ContextVersion, StateChange } from '../manager';
...
describe('ContextManager', () => {
  ...
  describe('Context Tracking', () => {
    ...
    it('should track bot state changes', () => {
      ...
      const currentState = contextManager.getGameState();
    ...
    it('should track inventory state changes', () => {
      ...
      const currentInventory = contextManager.getGameState().inventory;
  ...
  describe('Context Relevance Scoring', () => {
    ...
    it('should apply temporal relevance decay', () => {
      ...
      contextManager.updateContextRelevance(category);
      const initialRelevance = contextManager.getContextRelevance().get(category)?.score || 0;
      
      // Simulate time passing
      jest.advanceTimersByTime(60000); // 1 minute
      
      contextManager.updateContextRelevance(category);
      const decayedRelevance = contextManager.getContextRelevance().get(category)?.score || 0;
    ...
    it('should adjust context weights based on priority', () => {
      ...
      const initialWeight = contextManager.getContextWeights().get(category)?.baseWeight || 0;
      
      contextManager.adjustContextWeight(category, 0.5);
      const adjustedWeight = contextManager.getContextWeights().get(category)?.baseWeight || 0;
  ...
  describe('Context Versioning', () => {
    ...
    it('should allow rollback to previous versions', () => {
      ...
      expect(currentState.health).toBe(20);
    });
  });
