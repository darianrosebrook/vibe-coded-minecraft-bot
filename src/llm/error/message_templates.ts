...
export class ErrorMessageFormatter {
  private static readonly templates: Record<ParsingErrorType, ErrorMessageTemplate> = {
    'INVALID_COMMAND': {
    ...
    'AMBIGUOUS_INTENT': {
      ...
      message: 'Your command could mean multiple things. Please be more specific.',
      suggestions: [
    ...
    'LLM_SERVICE_ERROR': {
      title: 'Service Error',
      message: 'I\'m having trouble processing your command. Please try again later.',
      suggestions: [
  ...
  public static formatMessage(errorType: ParsingErrorType, context: ErrorMessageContext): string {
  ...
  public static getDetailedExplanation(errorType: ParsingErrorType, context: ErrorMessageContext): string {
    const template = this.templates[errorType];
    ...
    if (context.task) {
      ...
      if (context.task.parameters) {
        explanation += `Parameters: ${JSON.stringify(context.task.parameters, null, 2)}\n`;
      }
    }
