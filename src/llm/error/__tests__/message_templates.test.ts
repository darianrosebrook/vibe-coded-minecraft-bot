...
describe("ErrorMessageFormatter", () => {
  describe("formatMessage", () => {
    ...
    it("should include validation errors when present", () => {
      const message = ErrorMessageFormatter.formatMessage(
  ...
  describe("getRecoverySteps", () => {
    ...
    it("should return empty array for unknown error type", () => {
      const steps = ErrorMessageFormatter.getRecoverySteps(
        "UNKNOWN_ERROR" as ParsingErrorType
      );
  ...
  describe("getDetailedExplanation", () => {
    it("should include task details when present", () => {
      ...
      const explanation = ErrorMessageFormatter.getDetailedExplanation(
        "CONTEXT_MISMATCH",
        {
          task,
        }
    ...
    it("should include validation errors when present", () => {
      ...
      expect(explanation).toContain(
        "Invalid parameter: count must be positive"
      );
    });
  });
});
