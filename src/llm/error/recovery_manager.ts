import { ParsingErrorHandler, ParsingErrorType, ParsingErrorContext } from './parsingErrorHandler';
import { TaskParsingLogger } from '../logging/logger';
import { TaskContext } from '../parser/types';
...
export class ErrorRecoveryManager {
  ...
  private initializeStrategies(): void {
    ...
    this.addStrategy('LLM_SERVICE_ERROR', {
      ...
      priority: 1,
      maxRetries: 3,
      backoffStrategy: (attempt) => Math.min(1000 * Math.pow(2, attempt), 10000),
  ...
  private addStrategy(errorType: ParsingErrorType, strategy: RecoveryStrategy): void {
  ...
  public async attemptRecovery(error: Error, context: ParsingErrorContext): Promise<boolean> {
    ...
    for (const strategy of strategies) {
      ...
      try {
        ...
        this.logger.logError(error, {
          ...context,
          recoveryAttempt: `Attempting recovery with strategy ${strategy.name} (attempt ${retryCount + 1})`
        });
