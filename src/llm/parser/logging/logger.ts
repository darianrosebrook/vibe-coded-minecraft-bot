...
export class TaskParsingLogger {
  ...
  public logTypeValidationFailure(
    type: string,
    subType: string,
    errors: string[]
  ) {
    this.baseLogger?.error("Type validation failure", {
      type,
      subType,
      errors,
    });
  ...
  public logSubTypeValidationFailure(
    ...
    this.baseLogger?.error("Subtype validation failure", {
      type,
      subType,
      errors,
    });
  ...
  private addEntry(
    chainId: string,
    entry: Omit<LogEntry, "timestamp" | "sequenceId">
  ) {
    const chain = this.chains.get(chainId);
    if (chain) {
