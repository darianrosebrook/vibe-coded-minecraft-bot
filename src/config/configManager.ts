...
import { z } from 'zod';
import { Config, ConfigVersion, ConfigChangeCallback, baseConfigSchema } from '../types/config';
...
export class ConfigManager {
  ...
  private async loadConfig(): Promise<void> {
    try {
      ...
      const processedEnv = {
        ...process.env,
        MINECRAFT_PORT: process.env.MINECRAFT_PORT ? parseInt(process.env.MINECRAFT_PORT, 10) : undefined,
        BOT_VIEW_DISTANCE: process.env.BOT_VIEW_DISTANCE ? parseInt(process.env.BOT_VIEW_DISTANCE, 10) : undefined,
        BOT_CHAT_DISTANCE: process.env.BOT_CHAT_DISTANCE ? parseInt(process.env.BOT_CHAT_DISTANCE, 10) : undefined,
        OLLAMA_TEMPERATURE: process.env.OLLAMA_TEMPERATURE ? parseFloat(process.env.OLLAMA_TEMPERATURE) : undefined,
        OLLAMA_MAX_TOKENS: process.env.OLLAMA_MAX_TOKENS ? parseInt(process.env.OLLAMA_MAX_TOKENS, 10) : undefined,
        STORAGE_MAX_AGE: process.env.STORAGE_MAX_AGE ? parseInt(process.env.STORAGE_MAX_AGE, 10) : undefined,
        STORAGE_MAX_COMPLETED_AGE: process.env.STORAGE_MAX_COMPLETED_AGE ? parseInt(process.env.STORAGE_MAX_COMPLETED_AGE, 10) : undefined,
        STORAGE_CLEANUP_INTERVAL: process.env.STORAGE_CLEANUP_INTERVAL ? parseInt(process.env.STORAGE_CLEANUP_INTERVAL, 10) : undefined,
        STORAGE_MAX_HISTORY: process.env.STORAGE_MAX_HISTORY ? parseInt(process.env.STORAGE_MAX_HISTORY, 10) : undefined,
        LOG_MAX_SIZE: process.env.LOG_MAX_SIZE ? parseInt(process.env.LOG_MAX_SIZE, 10) : undefined,
        LOG_MAX_FILES: process.env.LOG_MAX_FILES ? parseInt(process.env.LOG_MAX_FILES, 10) : undefined,
        METRICS_PORT: process.env.METRICS_PORT ? parseInt(process.env.METRICS_PORT, 10) : undefined,
        BOT_SAFE_MODE: process.env.BOT_SAFE_MODE ? process.env.BOT_SAFE_MODE.toLowerCase() === 'true' : undefined,
        FEATURE_AUTO_EAT: process.env.FEATURE_AUTO_EAT ? process.env.FEATURE_AUTO_EAT.toLowerCase() === 'true' : undefined,
        FEATURE_AUTO_COLLECT: process.env.FEATURE_AUTO_COLLECT ? process.env.FEATURE_AUTO_COLLECT.toLowerCase() === 'true' : undefined,
        FEATURE_PATHFINDING: process.env.FEATURE_PATHFINDING ? process.env.FEATURE_PATHFINDING.toLowerCase() === 'true' : undefined,
        FEATURE_WORLD_TRACKING: process.env.FEATURE_WORLD_TRACKING ? process.env.FEATURE_WORLD_TRACKING.toLowerCase() === 'true' : undefined,
        METRICS_ENABLED: process.env.METRICS_ENABLED ? process.env.METRICS_ENABLED.toLowerCase() === 'true' : undefined
      };
      ...
      if (!result.success) {
        ...
        throw new Error(`Invalid configuration: ${JSON.stringify(errorDetails, null, 2)}`);
      }
  ...
  public async migrateConfig(oldConfig: Record<string, string>): Promise<Record<string, string>> {
