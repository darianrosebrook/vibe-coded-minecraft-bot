import { z } from 'zod';
import { Task, TaskType, TaskDependency, TaskParameters, TaskRequirements, ResourceRequirement, TaskPriority, TaskStatus } from '../types/task';
...
const taskSchema = z.object({
  ...
  parameters: z.union([
    ...
    z.object({
      ...
      target: z.object({
        type: z.enum(['lever', 'button', 'pressure_plate', 'redstone_torch', 'redstone_block', 'repeater', 'comparator']),
        position: z.object({
          x: z.number(),
          y: z.number(),
          z: z.number()
    ...
    z.object({
      queryType: z.enum(['inventory', 'position', 'nearby', 'status', 'help']),
      filters: z.object({
...
export class TaskValidator {
  ...
  public validateTaskParameters(type: Task['type'], parameters: unknown): { valid: boolean; errors: string[] } {
    try {
