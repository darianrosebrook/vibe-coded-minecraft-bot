...
class BatchedLogger {
  ...
  private static readonly FLUSH_INTERVAL = 10000; // Reduced to 10s for more frequent updates
  private static readonly MAX_BATCH_SIZE = 100; // Increased max batch size
  ...
  static flush(force: boolean = false) {
    ...
    // Group messages by type and level
    ...
    for (const [group, { count, level }] of groupedMessages.entries()) {
      ...
      switch (group) {
        ...
        case 'progress':
          ...
          break;
        case 'resource':
          message = `Found resources in ${count} locations`;
          break;
        case 'summary':
          message = `Completed processing ${count} chunks`;
          break;
        default:
          message = `Other operations: ${count}`;
      }
      
      // Use debug level for verbose operations, info for important ones
      const finalLevel = group === 'other' || group === 'progress' ? 'debug' : level;
  ...
  private processChunkData(chunkX: number, chunkZ: number): Vec3[] {
    ...
    for (let x = 0; x < 16; x++) {
      for (let z = 0; z < 16; z++) {
        for (let y = 0; y < 256; y++) {
          processedBlocks++;
          if (processedBlocks % 50000 === 0) {
            const progress = Math.round((processedBlocks / totalBlocks) * 100);
            // BatchedLogger.log(`Chunk (${chunkX}, ${chunkZ}) progress: ${progress}%`, 'debug');
          }
    ...
    BatchedLogger.log(`Chunk (${chunkX}, ${chunkZ}) processed: ${processedBlocks} blocks, ${valuableBlocksFound} valuable blocks found`, 'info');
  ...
  private processBlock(block: Block, position: Vec3): void {
    ...
    try {
      ...
      if (this.isValuableBlock(block.name)) {
        ...
        // Update metrics and batched logging
        metrics.blocksMined.inc({
          block_type: block.name
        });
        BatchedLogger.log(`Found ${block.name} at (${position.x}, ${position.y}, ${position.z}) in ${biomeName}`, 'info');
      }
    } catch (error) {
